import gsap from 'gsap';
import { SplitText} from 'gsap/all'
import { useGSAP } from '@gsap/react'

const About = () => {
 useGSAP(() => {
	const titleSplit = SplitText.create('#about h2', {
	 type: 'words'
	})
	
	const scrollTimeline = gsap.timeline({
	 scrollTrigger: {
		trigger: '#about',
		start: 'top center'
	 }
	})
	
	scrollTimeline
	 .from(titleSplit.words, {
		opacity: 0, duration: 1, yPercent: 100, ease: 'expo.out', stagger: 0.02
	})
	 .from('.top-grid div, .bottom-grid div', {
		opacity: 0, duration: 1, ease: 'power1.inOut', stagger: 0.04,
	}, '-=0.5')
 })
 
 return (
	<div id="about">
	 <div className="mb-16 md:px-0 px-5">
		<div className="content">
		 <div className="md:col-span-8">
			<p className="badge">Art of Dressing</p>
			<h2>
			 Where every detail matters <span className="text-white">-</span>
				from stitch to style.
			</h2>
		 </div>
		 
		 <div className="sub-content">
			<p>
			 Every piece we offer is chosen with care — from fabric to fit. That attention to detail turns everyday wear into something truly memorable.
			</p>
			
			<div>
			 <p className="md:text-3xl text-xl font-bold">
				<span>22</span> Years
			 </p>
			 <p className="text-sm text-white-100">
				of Fashion, Trust & Trend.
			 </p>
			</div>
		 </div>
		</div>
	 </div>
	 
	 <div className="top-grid">
		<div className="md:col-span-3">
		 <div  className="" />
		 <img src="/images/abt1.jpg" alt="grid-img-1" />
		</div>
		
		<div className="md:col-span-6">
		 <div  className="" />
		 <img src="/images/abt2.jpg" alt="grid-img-2" />
		</div>
		
		<div className="md:col-span-3">
		 <div  className="" />
		 <img src="/images/abt5.jpg" alt="grid-img-5" />
		</div>
	 </div>
	 
	 <div className="bottom-grid">
		<div className="md:col-span-8">
		 <div  className="" />
		 <img src="/images/abt3.jpg" alt="grid-img-3" />
		</div>
		
		<div className="md:col-span-4">
		 <div  className="" />
		 <img src="/images/abt4.jpg" alt="grid-img-4" />
		</div>
	 </div>
	 
	</div>
 )
}
export default About